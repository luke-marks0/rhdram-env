...
`RD`, `WR`, and `WAIT` are the only policy-issuable ops. `ACT`, `PRE`, `REF`, and
`RFM` are generated by the memory controller in response to the request stream and
by the refresh/mitigation logic; a policy cannot issue them, and `dram.issue`
rejects them with `ILLEGAL_COMMAND`. They remain visible in the issued-event stream
(§5) and drive disturbance accounting there.
