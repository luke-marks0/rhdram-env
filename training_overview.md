...
The supported PoC recipe and commands are now in [poc_training.md](poc_training.md).
This page describes the broader historical harness, including optional hard bands
and legacy single-shot/evaluation paths. The PoC uses `eval_poc.py`, a three-tool
server surface, exact sampled-token masks, and a separate final benchmark split.
